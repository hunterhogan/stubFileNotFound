from _typeshed import Incomplete
from afdko import convertfonttocid as convertfonttocid, fdkutils as fdkutils, ufotools as ufotools
from typing import Any, Literal

__doc__: str
__version__: str
__methods__: str
__usage__: Incomplete
__help__: Incomplete
kInputFont: str
kOutputFont: str
kBold: str
kItalic: str
kFeature: str
kGSUBStub: str
kFMB: str
kGOADB: str
kRelease: str
kDoSubr: str
kDoAlias: str
kDoSubset: str
kRenumber: str
kMaxSubrs: str
kSetfsSelectionBitsOn: str
kSetfsSelectionBitsOff: str
kSetOS2Version: str
kNotdef: str
kAddSymbol: str
kSerif: str
kXUID: str
kConvertToCID: str
kSuppressHintWarnings: str
kSuppressWidthOptimization: str
kStubCmap4: str
kSuppressKernOptimization: str
kMacScript: str
kMacLang: str
kMacCMAPPath: str
kHUniCMAPPath: str
kUVSPath: str
kWriteWrongBacktrack: str
kDoubleMapGlyphs: str
kDefaultCharAsSpace: str
kUseOldNameID4: str
kOmitMacNames: str
kOverrideMenuNames: str
kLicenseCode: str
kDefaultFontPathList: Incomplete
kDefaultFeaturesList: Incomplete
kDefaultOptionsFile: str
kDefaultFMNDBPath: str
GOADB_NAME: str
kAddStubDSIG: str
kShowFinalNames: str
kVerboseWarnings: str
kOptionNotSeen: int
kMOTFOptions: Incomplete
kSkipOptions: Incomplete
kFileOptList: Incomplete
kFileOptPrefix: str

class FDKEnvironmentError(Exception): ...
class MakeOTFOptionsError(Exception): ...
class MakeOTFShellError(Exception): ...
class MakeOTFRunError(Exception): ...

class MakeOTFParams:
	makeotfPath: Incomplete
	txPath: Incomplete
	currentDir: Incomplete
	optionFilePath: Incomplete
	newOptionFilePath: Incomplete
	fontinfoPath: Incomplete
	saveOptions: str
	fontDirPath: str
	tempFontPath: Incomplete
	ROS: Incomplete
	seenOS2v4Bits: Incomplete
	srcIsTTF: int
	srcIsUFO: int
	ufoFMNDBPath: Incomplete
	ufoGAODBPath: Incomplete
	verbose: bool
	def __init__(self) -> None: ...

def readOptionFile(filePath, makeOTFParams, optionIndex) -> tuple[Literal[True], Any] | tuple[bool, Any]:
	""" Paths in this file are stored relative to it's directory.
	However, we want to set the paths relative to the current directory.
	"""
def writeOptionsFile(makeOTFParams, filePath) -> None:
	""" Note that we have to convert the current options from
	relative to the current directory to relative to the fpr
	file we are writing."""
def saveOptionsFile(makeOTFParams) -> None: ...
def setOptionsFromFontInfo(makeOTFParams) -> None:
	""" This sets options from the fontinfo file
		*if they have not already been set *
	"""
def getRelativeDirPath(absPath1, abspath2) -> None:
	"""
	Return a relative path to the first dir from the second directory.
	If the paths have no common prefix, return None
	"""
def getOptions(makeOTFParams, args) -> None:
	"""
	Fill in the makeotf parameters from first the defaults,
	then the project file, and finally any other options.
	We require that the -fp option, if used, be the first;
	that way the project file is read in before any other
	options are applied.

	All file paths are taken to be relative to the current
	working directory. However, relative paths saved in a
	project file are relative to the project file location.
	"""
def checkInputFile(makeOTFParams) -> None: ...
def lookUpDirTree(fileName) -> str | None:
	"""
	This is called when we are using a default name for either the
	FontMenuNameDB or the GlyphOrderAndAliasDB files.
	These are often located several dir levels above the font file,
	as they are shared by the font family.
	"""
def getROS(fontPath) -> tuple[None, None, None] | tuple[None, str | Any, None] | tuple[str | Any, str | Any, str | Any]:
	"""
	Check to see if font is a CID-keyed font by
	looking for the keys Registry, Ordering, Supplement.
	We are looking for the text:
	  /Registry (Adobe)  def
	  /Ordering (Japan1) def
	  /Supplement 3 def

	"""

def setCIDCMAPPaths(makeOTFParams, Reg, Ord, Sup) -> bool:
	"""This is called when one or more of the three Adobe CMAP files
	wanted for CID fonts have not been specified on the command-line.
	Only the Uni-H file is required.
	"""

RE_FEATURE: Incomplete
RE_INCLUDE: Incomplete

def checkIfVertInFeature(featurePath) -> bool:
	...
def psname_in_fmndb(ps_name, fmndb_path) -> bool:
	"""
	Check the FontMenuNameDB file for the presence of a specific PostScript
	name. Returns a boolean.
	"""
def setMissingParams(makeOTFParams) -> None: ...
def convertFontIfNeeded(makeOTFParams) -> None: ...
def get_font_psname(font_path, is_ufo: bool = False): ...
def updateFontRevision(featuresPath, fontRevision) -> None:
	"""
	Set or increment FontRevision value in fea file.

	supplied 'fontRevision' kind specifies behavior:
	- float/decimal means "set to" supplied value, e.g.:
	  .fea FontRevision = '2.102', fontRevision == '1.100', result = '1.100'

	- integer value means "increment minor by this value (thousandths)":
	  .fea FontRevision = '1.230', fontRevision == '5', result = '1.235'
	  .fea FontRevision = '1.1', fontRevision == '1', result = '1.101'
	  .fea FontRevision = '1.02', fontRevision == '31', result = '1.051'

	Constraint: resulting FontRevision must be in range 0.000 - 32767.000.
	This means:
	   - for float case, 0.000 < float(fontRevision) <= 32767.000
	   - for int case, fractional part + increment <= .999
	   - if supplied values would overflow, a message is printed and
		 the feature file is not updated.
	"""

def checkFSTypeValue(FSType, outputPath) -> None: ...
def getSourceGOADBData(inputFilePath) -> list[Any]:
	...

def getGOADBData(goadbPath) -> list[Any]:
	...

def compareSrcNames(srcGOADBList, realGOADBList) -> tuple[Literal[1], list[Any], list[Any]] | tuple[Literal[0], list[Any], list[Any]]:
	...

def writeTempGOADB(srcGOADBList) -> str:
	...
def copyTTFGlyphTables(inputFilePath, tempOutputPath, outputPath) -> None: ...
def fixPost(glyphList, font) -> None:
	"""
	Replace 'post' table
	"""
def update_table_items(table_tag, items_list, font1, font2) -> None:
	"""
	font1 will get new values from font2
	"""
def fixHead(temp_font, font) -> None:
	"""
	temp_font is an OTF/CFF font.
	font is a TTF font.
	Need to update the head values. Can't just copy the entire table
	from the OTF temp font, as as some of the head table values control
	interpretation of the glyf data.
	"""
def fixHhea(temp_font, font) -> None: ...
def copy_tables(temp_font, font, tags) -> None: ...
def adjustPaths(makeOTFParams):
	"""
	Change file paths to be relative to fontDir,
	if possible, else to absolute paths.
	(With the exception of fmndbFilePath and tempFontPath which are
	always absolute paths.)
	"""

byEntryOrder: Incomplete

def runMakeOTF(makeOTFParams) -> None: ...
def CheckEnvironment() -> None: ...
def main(args: list[str] | None = None) -> Literal[1, 2] | None:
	...
