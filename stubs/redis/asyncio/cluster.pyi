...
class RedisCluster(AbstractRedis, AbstractRedisCluster, AsyncRedisClusterCommands):
    ...
    async def transaction(self, func: Coroutine[None, 'ClusterPipeline', Any], *watches: Any, **kwargs: Any) -> Any:
        """
...
class ClusterPipeline(AbstractRedis, AbstractRedisCluster, AsyncRedisClusterCommands):
    ...
    async def unlink(self, *names: Any) -> None: ...
...
class ExecutionStrategy(ABC, metaclass=abc.ABCMeta):
    ...
    def multi(self) -> Any:
        ...
        """
    @abstractmethod
    async def watch(self, *names: Any) -> Any:
        """
        Watch given keys.
...
class AbstractStrategy(ExecutionStrategy, metaclass=abc.ABCMeta):
    ...
    async def initialize(self) -> ClusterPipeline: ...
    def execute_command(self, *args: KeyT | EncodableT, **kwargs: Any) -> ClusterPipeline: ...
    def _annotate_exception(self, exception: Any, number: Any, command: Any) -> None:
        """
...
class TransactionStrategy(AbstractStrategy):
    ...
    async def _immediate_execute_command(self, *args: Any, **options: Any) -> Any: ...
    async def _get_connection_and_send_command(self, *args: Any, **options: Any) -> Any: ...
    async def _send_command_parse_response(self, connection: Connection, redis_node: ClusterNode, command_name: Any, *args: Any, **options: Any) -> Any:
        """
        Send a command and parse the response
        """
    reinitialize_counter: int
    async def _reinitialize_on_error(self, error: Any) -> None: ...
    ...
    def mset_nonatomic(self, mapping: Mapping[AnyKeyT, EncodableT]) -> ClusterPipeline: ...
    async def execute(self, raise_on_error: bool = True, allow_redirections: bool = True) -> list[Any]: ...
    async def _execute_transaction_with_retries(self, stack: list['PipelineCommand'], raise_on_error: bool) -> Any: ...
    async def _execute_transaction(self, stack: list['PipelineCommand'], raise_on_error: bool) -> Any: ...
    _command_queue: Incomplete
    async def reset(self) -> None: ...
    def multi(self) -> None: ...
    async def watch(self, *names: Any) -> Any: ...
