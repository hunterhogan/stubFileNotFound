...
def focal_loss_with_logits(output: torch.Tensor, target: torch.Tensor, gamma: float = 2.0, alpha: float | None = 0.25, reduction: str = 'mean', normalized: bool = False, reduced_threshold: float | None = None, eps: float = 1e-06) -> torch.Tensor:
    ...
    Args:
        ...
        reduction (string, optional): Specifies the reduction to apply to the output:
            ...
            Note: :attr:`size_average` and :attr:`reduce` are in the process of
            being deprecated, and in the meantime, specifying either of those two args
            will override :attr:`reduction`. 'batchwise_mean' computes mean loss per
            sample in batch. Default: 'mean'
        normalized: Use normalized focal loss (https://arxiv.org/pdf/1909.07829.pdf)
        reduced_threshold: Compute reduced focal loss (https://arxiv.org/abs/1903.01347)
...
def wing_loss(output: torch.Tensor, target: torch.Tensor, width: int = 5, curvature: float = 0.5, reduction: str = 'mean'):
    ...
    """
def label_smoothed_nll_loss(lprobs: torch.Tensor, target: torch.Tensor, epsilon: float, ignore_index=None, reduction: str = 'mean', dim: int = -1) -> torch.Tensor:
