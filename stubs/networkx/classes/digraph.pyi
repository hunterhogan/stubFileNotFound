...
class DiGraph(Graph[_Node]):
    ...
    **Subclassing Example**
    ...
    def in_degree(self) -> InDegreeView:
        ...
        This object provides an iterator for (node, in_degree) as well as
        lookup for the in_degree for a single node.
        ...
        If a single node is requested
        deg : int
            In_degree of the node

        OR if multiple nodes are requested
        nd_iter : iterator
            The iterator returns two-tuples of (node, in_degree).
    ...
    def out_degree(self) -> OutDegreeView:
        ...
        The weighted node degree is the sum of the edge weights for
        edges pointing out of that node.

        This object provides an iterator for (node, out_degree) as well as
        lookup for the out_degree for a single node.
        ...
        If a single node is requested
        deg : int
            Out_degree of the node

        OR if multiple nodes are requested
        nd_iter : iterator
            The iterator returns two-tuples of (node, out_degree).
    ...
    def to_undirected(self, as_view: bool = False) -> Graph[_Node]:
     ...
     Notes
     -----
     This method can also accept a `reciprocal` parameter (bool, default=False).
     If True, only edges that appear in both directions in the original digraph
     will be kept in the undirected graph.
