...
class Graph(Collection[_Node]):
	...
	def to_directed_class(self) -> type['DiGraph[_Node]']:
		...
		to_directed(), this provides the class that will be used.
	...
	def to_undirected_class(self) -> type['Graph[_Node]']:
		...
		When creating an undirected version of the graph with functions like
		to_undirected(), this provides the class that will be used.
	...
	def name(self) -> str:
		...
		keyed by the string `"name"`. As well as being stored as a graph
		attribute, this string is returned by the `__str__` method of the
		graph class. If `name=None`, a unique string identifier is chosen.
		"""
		...
	...
	def remove_nodes_from(self, nodes: Iterable[_Node]) -> None:
		...
		nodes : iterable container
			A container of nodes (list, dict, set, etc.).
			If a node in the container is not in the graph it is silently
			ignored.
		"""
		...
	...
	def nodes(self) -> NodeView:
		...
		The NodeView provides set-like operations, a dictionary interface,
		and a convenient way to iterate through node attributes.
		...
		NodeView
			...
			attribute dict lookup and iteration over nodes data.
	...
	def add_edges_from(self, ebunch_to_add: Iterable[_EdgePlus[_Node]], **attr: Any) -> None:
		...
		Edge attributes specified in ebunch_to_add as 3-tuples (u, v, d) take
		precedence over attributes specified via keyword arguments.
